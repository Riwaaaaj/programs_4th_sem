...
public:
  ...
  ~LinkedList() { // Memory deallocation by traversing through all the nodes, as
                  // the new nodes were created using DMA.
    Node *current = head;
    while (current != NULL) {
      Node *temp = current;
...
/*
Output when the line 216 to 220 are commented:
